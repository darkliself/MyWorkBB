...
void Compliant_Standards(){
    ...
    if(!String.IsNullOrEmpty(CompliantStandardsRef)){
        switch(CompliantStandardsRef.Split(',').Count()){
            case 1:
            CompliantStandards = $"Meets or exceeds {CompliantStandardsRef.TrimEnd(',')} standards";
            break;
            case var a when a > 1:
            CompliantStandards = $"Meets or exceeds {CompliantStandardsRef.Split(", ").FlattenWithAnd(10, ", ")} standards";
            break;
        }
    }
...
void Warr_anty(){
    ...
    // IF A[5312].Value LIKE "Refurbished" 
    // THEN A[430].Value.Erase("Warranty").Erase("limited").Replace(" years", "-year").Replace(" year", "-year").Replace(" days", "-day").Postfix(" Non-mfg. warranty through Vendor. This product is NOT warrantied through ")_SKU.Brand
    ...
    R($"cnet_common_{refer}").HasValue() ? R($"cnet_common_{refer}").Replace("<NULL>", "").Text : "";
        ...
        else if(refer.ToLower().Contains("replacement")){
            result = "Lifetime manufacturer limited replacement  warranty";
            break;
        }
        else if(refer.ToLower().Contains("ifetime manufacturer")){
...
void Graphics_All(){
    ...
    if(VideoOutput_GraphicsProcessor.HasValue()){
        ...
        switch(VideoOutput_GraphicsProcessor){
            ...
            Graphics = $"{VideoOutput_GraphicsProcessor.FirstValue().ToString().Split('/').Last().TrimStart() + " graphics and"}{Size}{UnitsSize}{Technology} dedicated graphics memory ensure ultimate gaming experience";
            break;
            ...
            Graphics = $"{GraphicsProcessor} provides everyday image quality for Internet usage, basic photo editing, and casual gaming";
            break;
...
void PrintersConnectivity(){
    ...
    && InterfaceRequired_Type.HasValue()){
        result = $@"Network-ready wired {Coalesce(InterfaceRequired_Type.Where("%lan%").Match(23, 6836).Values(" x ").Select(s => s.RegexReplace(@"(.*)\sx\s(\d*).*", "##$2")).First(), "##1")} x {Connectivity_Interface.Where("%LAN%").First().Value()} Ethernet and {InterfaceRequired_Type.WhereNot("%LAN%").Match(23, 6836).Values(" x ").Select(s => s.RegexReplace(@"(.*)\sx\s(\d*).*", "##$2 x $1")).Flatten(", ").Replace("<>", "##1")} connectivity";
    }
    else if(Connectivity_Interface.HasValue()
    && Connectivity_Interface.WhereNot("%Bluetooth%").Any()
    && Connectivity_Interface.Where("%LAN%").Any()){
...
void PrintersInputLoadingFeature(){
    ...
    var MediaCapacity = TotalMediaCapacity.HasValue() ? $"{TotalMediaCapacity.FirstValue()} {TotalMediaCapacity.Units.First().Name.Replace("page", "pages").Replace("sheet", "sheets")}" : "";
    ...
    && !String.IsNullOrEmpty(MediaCapacity)){
        result = $"{MediaCapacity} input capacity and a single-sheet bypass tray; types of media supported include {Shorten(Coalesce(DocumentMediaHandling_SupportedMediaType, Printer_MediaType).FirstValue().ToString(), 40)}";
    }
    ...
    && Coalesce(DocumentMediaHandling_SupportedMediaType, Printer_MediaType).HasValue()
    && !String.IsNullOrEmpty(MediaCapacity)){
        result = $"{MediaCapacity} input capacity; types of media supported include {Shorten(Coalesce(DocumentMediaHandling_SupportedMediaType, Printer_MediaType).FirstValue().ToString(), 40)}";
    }
    else if(!String.IsNullOrEmpty(PaperInputCapacityRef)
    && !String.IsNullOrEmpty(MediaCapacity)){
        ...
        result = $"Types of media supported include {Shorten(Coalesce(DocumentMediaHandling_SupportedMediaType, Printer_MediaType).FirstValue().ToString(), 175)}";
    }
    if(!String.IsNullOrEmpty(result)){
...
void PrintersProcessorMemory(){
    ...
    && FaxMachine_TotalMemoryCapacity.HasValue()){
        result = $"{PrinterMemoryRef}, up to {FaxMachine_TotalMemoryCapacity.FirstValue()} pages fax memory";
    }
    else if(!String.IsNullOrEmpty(PrinterProcessorRef)){
        result = $"Processor speed is {PrinterProcessorRef} to handle large printing jobs";
    }
    else if(!String.IsNullOrEmpty(PrinterMemoryRef)){
        result = $"{PrinterMemoryRef} printer memory provides convenient, reliable printing of your most important documents";
    }
...
void AutomaticPrintingFeatures(){
    ...
    && A[2712].HasValue(DocumentMediaHandlingDetails_Type.Where("ADF").Match(2712).Values().Select(s => s.RegexReplace(@"(.*)\s(\d*).*", "$2")).First().ToString()) ?
    $" with {A[2712].FirstValue()} {A[2712].Units.First().Name} auto feeder" : "";
...
void PrintersDisplay(){
    ...
    else if(Chassis_BuiltInDevices.HasValue("%touch screen%")
    && Chassis_BuiltInDevices.HasValue("%display%")){
...
void PrintersScanningCapabilities(){
    ...
    var OpticalResolution = Scanning_OpticalResolution.HasValue() ? $"{Scanning_OpticalResolution.FirstValue()} optical resolution, " : "";
...
void TabletsCameraDetails(){
    ...
    && !String.IsNullOrEmpty(RearCameraRef)){
        ...
        var LensAperture = DigitalCamera_LensAperture.HasValue() ? $", {DigitalCamera_LensAperture.FirstValue().Replace("/", "").ToUpper()} (rear)/" : "";
...
void TabletsImputsAndOutputs(){
    var result = "";
    var Interfaces_Interface = Coalesce(A[5791]); // Interfaces - Interface
    var InterfaceProvided_Interface = Coalesce(A[6040]); // Interface Provided - Interface

    if(Interfaces_Interface.HasValue()){
        result = Interfaces_Interface.Values.Match(5792, 5791).Values(" x ").Select(s => s
        .RegexReplace(@"(.*)\sx\s(\d*).*", "$2 x $1")
        ).FlattenWithAnd()
        .Replace(" x 1", "")
        .Replace("1 x ", "")
        .Replace("  x ", "")
        ...
        .RegexReplace("((.* x )*headphones$)|headphones", "headphone jack lets you connect your PC tablet to a sound system")
        .RegexReplace(@"((.* x )*headphones mini jack \(3\.5mm\))", "$1 lets you connect your tablet to a sound system")
        ...
        .RegexReplace("((.* x )*headset)", "headset jack lets you connect your PC tablet to a sound system")
        .RegexReplace("((.* x )*Lightning)", "enjoy revolutionary data transfer speeds with a Lightning connector")
        .Replace("2 x Micro-USB port", "2 x Micro-USB ports").ToUpperFirstChar();
    }
    else if(InterfaceProvided_Interface.HasValue()){
        result = InterfaceProvided_Interface.Values.Match(10, 6040).Values(" x ").Select(s => s
        .RegexReplace(@"(.*)\sx\s(\d*).*", "$2 x $1")
        ).Flatten("; ")
        .Replace(" x 1", "")
        .Replace("1 x ", "")
        .Replace("  x ", "")
        .Replace("Smart Connector", "connect to your device with a Smart Connector, eliminating the need for Bluetooth pairing")
        .RegexReplace("((.* x )*Micro-USB)", "features a $1 port for simple charging and file transfers")
        .RegexReplace("((.* x )*headphones$)|headphones", "headphone jack lets you connect your PC tablet to a sound system")
        .RegexReplace(@"((.* x )*headphones mini jack \(3\.5mm\))", "$1 lets you connect your tablet to a sound system")
        .RegexReplace("((.* x )*USB-C.*)", "tablet has $1 interfaces for external device connection")
...
void Headset_Microphone_Type_Use(){
    ...
    if(HeadsetMicrophoneTypeRef.ToLower().In("%headset%")){
        HeadsetMicrophoneTypeUse = $"{HeadsetMicrophoneTypeRef.Text.ToLower().ToUpperFirstChar()} is perfect for everyday use with compatible electronic devices";
    }
...
void Type_Of_Cable_Interface_Use(){
    ...
    var VideoCableTypeRef = R("SP-18471").HasValue() ? R("SP-18471").Text : 
        R("cnet_common_SP-18471").HasValue() ? R("cnet_common_SP-18471").Text : "";// Video Cable Type
    ...
    && !String.IsNullOrEmpty(InterfaceRef)){
        TypeOfCableInterfaceUse = $"This {VideoCableTypeRef.ToLower()} is ideal to connect your {InterfaceRef}-compatible devices";
    }
...
void Connectors_Plating(){
    ...
    && !String.IsNullOrEmpty(ConnectorGenderRef)){
        var AdditionalFeatures = Cables_Cable_AdditionalFeatures.HasValue() && Cables_Cable_AdditionalFeatures.Where("corrosion-resistant connectors").Any() ? $" {Cables_Cable_AdditionalFeatures.Where("corrosion-resistant connectors").First().Value().Replace("yes"," with corrosion resistance").Replace("No","")}" : "";
        var CableFeatures = SystemPowerCables_Cable_CableFeatures.HasValue() && SystemPowerCables_Cable_CableFeatures.Where("corrosion-resistant connectors").Any() ? SystemPowerCables_Cable_CableFeatures.Where("corrosion-resistant connectors").First().Value().Replace("yes"," with corrosion resistance").Replace("No","") : "";
...
void Hard_Drive_Type(){
    ...
    if(HardDrive_Capacity.HasValue()){
        if(HardDrive_Capacity.Units.First().Name.NotIn("GB", "TB")){
            ...
            $"{HardDrive_Capacity.FirstValue()}{HardDrive_Capacity.Units.First().Name} eMMC flash hard drive internal storage space";
        }
...
void USB_Ports_Of_Desktop(){
    var result = "";
    var USBPortsOfDesktopRef = R("SP-4003").HasValue() ? R("SP-4003").Replace("<NULL>", "").Text :
        R("cnet_common_SP-4003").HasValue() ? R("cnet_common_SP-4003").Replace("<NULL>", "").Text : "";

    if(!String.IsNullOrEmpty(USBPortsOfDesktopRef)){
        result = $"Use {USBPortsOfDesktopRef} ports for lightning data movement speed";
    }
    if(!String.IsNullOrEmpty(result)){
...
void Additional_PortsInterface(){
    var result = ""; // Additional Ports/Interface
    var DesktopOtherPortsRef = R("SP-4004").HasValue() ? R("SP-4004").Replace("<NULL>", "").Text :
        R("cnet_common_SP-4004").HasValue() ? R("cnet_common_SP-4004").Replace("<NULL>", "").Text : "";

    if(!String.IsNullOrEmpty(DesktopOtherPortsRef)){
        var tmp = DesktopOtherPortsRef.Split(" ").Select(o => Coalesce(o).ExtractNumbers().Any() ? "##" + o : o).Flatten(" ");
        result = $"Built-in {tmp} ports for lightning data movement speed output to connect to TVs or multiple displays for stunning HD entertainment";
    }
    if(!String.IsNullOrEmpty(result)){
...
void TrueColor_FileFolder_Material(){
    ...
    void Number_Of_Tabs(){
        ...
        var FilingSystem_Features = Coalesce(A[5945]); //Filing System - Features
        var FilingSystem_TabQty = Coalesce(A[5946]); //Filing System - Tab Qty
        
        if(FilingSystem_Features.HasValue() && FilingSystem_Features.Where("two-ply tab", "reinforced tab").Count() == 2){
            result = "Reinforced 2-ply tabs provide extra strength where you need it most";
        }
        else if(Tabs.Equals("No Tabs") || String.IsNullOrEmpty(Tabs)
        && !String.IsNullOrEmpty(FileFolderType)){
            result = $"{FileFolderType.ToLower().ToUpperFirstChar()} for easy labeling";
        }
        else if(FilingSystem_TabQty.HasValue("1")
        && FilingSystem_Features.HasValue("%/%tab%")){
            result = $"{FilingSystem_Features.Where("%/%tab%").Flatten(" ").ExtractNumbers().Flatten("/")}-cut {Coalesce(FileFolderType, "tab")} keeps filing and reordering files simple";
        }
        else if(FilingSystem_TabQty.HasValue("1")){
            result = $"Single {Coalesce(FileFolderType, "tab")} keeps filing and reordering files simple";
        }
        else if(FilingSystem_TabQty.HasValue()
        && FilingSystem_TabQty.FirstValue() >= 5
        && FilingSystem_Features.HasValue("%/%tab%")){
            result = $"{FilingSystem_Features.Where("%/%tab%").Flatten(" ").ExtractNumbers().Flatten("/")}-cut {Coalesce(FileFolderType, "tab")}s allow ample space for labeling to keep your files organized";
        }
        else if(FilingSystem_TabQty.HasValue()
        && FilingSystem_TabQty.FirstValue() >= 5){
            result = $"Five {Coalesce(FileFolderType, "tab")}s allow ample space for labeling to keep your files organized";
        }
        else if(FilingSystem_TabQty.HasValue()
        && FilingSystem_TabQty.FirstValue() > 1
        && FilingSystem_Features.HasValue("%/%tab%")){
            var Tab = !String.IsNullOrEmpty(Tabs) ? $"{Tabs} " : "";
            result = $"{Tab}{FilingSystem_Features.Where("%/%tab%").Flatten(" ").ExtractNumbers().Flatten("/")}-cut {Coalesce(FileFolderType, "tab")}s allow ample space for labeling to keep your files organized";
        }
        else if(FilingSystem_TabQty.HasValue()
        && FilingSystem_TabQty.FirstValue() > 1){
            var Tab = !String.IsNullOrEmpty(Tabs) ? $"{Tabs} " : "";
            result = $"{Tab}{Coalesce(FileFolderType, "tab")}s allow ample space for labeling to keep your files organized";
        }
        if(!String.IsNullOrEmpty(result)){
            Add($"NumberOfTabs⸮{result}");
        }
    }
...
void CompatibleDevices() {
    ...
    if (cp.HasValue()) {
        result = $"Compatible with: {cp.Text.Split(", ").Take(5).Flatten(", ")}";
    }
    if (!String.IsNullOrEmpty(result)) {
...
void  LabelMakerBatterySize() {
    ...
    if (!String.IsNullOrEmpty(batterySize) && !batterySize.ToLower().Equals("none")) {
        if (count.HasValue() && count.Values.ExtractNumbers().Any() && count.Values.ExtractNumbers().First() > 1) {
           result = $"Runs on ##{count.FirstValue()} x {batterySize.Replace("Lithium Ion", "lithium ion")} batteries";
        }
        else {
            result = $"Runs on {batterySize.Replace("Lithium Ion", "lithium ion")} battery";
        }
    }
    if (!String.IsNullOrEmpty(result)) {
